...
# ========================================= #
# RL Training Loop "mult_env_vector"        #
# ========================================= #

@register_trainer("mult_env_vector")
class RLTrainerServer(BaseRLTrainerServer):
    ...
    async def global_training_loop(self):
        ...
        Log.logInfo("Starting global-training-loop...")
        ...
        try:
            ...
            state_dim = first_env.state_dim * self.config["NUM_STACK"]             
            stacker_shape = (self.config["NUM_STACK"], first_env.state_dim)        
            rollout_buffer = RolloutBuffer(buffer_size = self.config["BUFFER_SIZE"], buffer_shape = (state_dim,),device=self.config["DEVICE"])

            policy = make_policy("mlp_shared", input_dim = state_dim, action_dim = first_env.action_dim, hidden_sizes = self.config["HIDDEN_SIZES"])
            ppo_trainer = PPO(model = policy, config = self.config)

            env_stackers = {env: StateStacker(stacker_shape, num_stack=self.config["NUM_STACK"], device=self.config["DEVICE"]) for env in self.active_envs}
            ...
            for episode in range(self.config["EPISODES"]):
                ...
                current_stacked_states = [env_stackers[env].reset(raw_frames[i]).flatten() for i, env in enumerate(self.active_envs)]
                ...
                while not rollout_buffer.is_full() and counter_env_endings < self.config["NUM_ENVS"]:
                    ...
                    for i, env in enumerate(self.active_envs):
                        ...
                        next_stacked_state = stacker.push(next_raw_frame).flatten()
                        ...
                        if terminated or truncated:
                            ...
                            fresh_stacked_state = stacker.reset(fresh_raw_frame).flatten()
                ...
                _, _, next_value = ppo_trainer.select_action(last_state_tensor)
